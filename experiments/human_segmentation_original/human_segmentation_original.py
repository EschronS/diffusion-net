...
pretrain_path = os.path.join(
    base_path, "pretrained_models/human_seg_{}_4x128.pth".format(input_features)
)
# model_save_path = os.path.join(
#     base_path, "data/saved_models/human_seg_{}_4x128.pth".format(input_features)
# )
...
test_dataset = HumanSegOrigDataset(
...
if train:
    train_dataset = HumanSegOrigDataset(
...
if train:
    ...
    for epoch in range(n_epoch):
        ...
        # if one 10th of epochs is done save an intermediate model
        if epoch % np.round(n_epoch / 10) == 0:
            inter_model_save_path = os.path.join(
                base_path,
                f"data/saved_models/human_seg_{input_features}_4x128_epoch_{epoch}.pth",
            )
            print(f" ==> saving epoch {epoch} model to " + inter_model_save_path)
            torch.save(model.state_dict(), inter_model_save_path)

    model_save_path = os.path.join(
        base_path,
        f"data/saved_models/human_seg_{input_features}_4x128_epoch_{n_epoch}.pth",
    )
    print(" ==> saving last model to " + model_save_path)
    torch.save(model.state_dict(), model_save_path)

    np_acc_list = np.array(acc_list)
    with open("train_and_test_acc_pickle", "bw") as file:
        pickle.dump(np_acc_list, file)
